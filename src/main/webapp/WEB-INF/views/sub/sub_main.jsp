...
function post_updateSchedule(info, stbn, stbn2){
	$.ajax({
...
function draw_reservation_record_table(info){
	...
	if(json.list.length == 0){
		str += "<tr><td colspan='10'>등록된 정보가 없습니다.</td></tr>";
	}else{
...
$(function(){
	...
	$(document).on("click", "#calendar td:not(.tr_not > td)", function(){
		...
		$(this).not(".tr_not > td").css({"border":"2px solid #8f8f8f"});
	...
	$(document).on("click", ".doctor_time_table .reservation_register_btn", function(){
		...
		str = "<span>"+pname+"("+chart_no+")님 진료일정추가</span><input type='hidden' name='pno' value='"+pno+"'><input type='hidden' name='pname' value='"+pname+"'><input type='hidden' name='chart_no' value='"+chart_no+"'>";
	...
	//진료/치료예약view에서 예약등록, 예약접수, 취소 버튼 기능
	$(document).on("click", ".popup_content2 .popup_reservation_register_btn_wrap > p", function(){
		...
		if(idx == 0 || idx == 1){
			...
			$(".popup_clinic_reservation_register input").val("");
			$(".popup_therapy_reservation_register input").val("");
			$(".popup_clinic_reservation_register > table tr > td > select[name='rtime_minute'] > option").eq(0).prop("selected", true);
			$(".popup_therapy_reservation_register > table tr > td > select[name='rtime_minute'] > option").eq(0).prop("selected", true);
			$(".popup_clinic_reservation_register .fix_clinic_res_tr").css("display", "none");
			$(".popup_therapy_reservation_register .fix_therapy_res_tr").css("display", "none");
	...
	$(document).on("click", ".patient_waitingRes_tag", function(){
		...
		if(wrVO.rtype == 'nc'){
			...
			$(".popup_clinic_reservation_register > .popup_reservation_register_btn_wrap").append("<p>대기예약취소<input type='hidden' name='no' value='"+no+"'></p>");
			...
			$(".popup_therapy_reservation_register > .popup_reservation_register_btn_wrap").append("<p>대기예약취소<input type='hidden' name='no' value='"+no+"'></p>");
	...
	$(".popup_sch_update_btn_wrap > p").click(function(){
		...
		if(btn_idx == 0){
			...
			var ename = $(".popup_reservation_schedule_update > table tr:nth-child(4) > td select[name='emp'] > option:selected").text();
			var clinic = $(".popup_reservation_schedule_update > table tr:nth-child(5) > td select[name='clinic']").val();
			var clinic_name = $(".popup_reservation_schedule_update > table tr:nth-child(5) > td select[name='clinic'] > option:selected").text();
			...
			var reception_info = regDate+" "+regTime+" "+$("#session_login_name").val();
			...
			var info = {pno:pno, rtype:rtype, fix_day_start:fix_day_start, fix_day_end:fix_day_end, req_day_start:req_day_start, req_day_end:req_day_end, fix_day:fix_day, rtime:rtime, eno:emp, ename:ename, clinic:clinic, reception_info:reception_info, result_memo:result_memo};
